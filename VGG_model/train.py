...
def train(retain_flag = True):
    ...
    with tf.Graph().as_default():
        ...
        valid_gallery_batch, valid_gallery_label = vgg.valid_batch_inputs(
            train_flags.dataset_valid_gallery_csv_file_path, train_flags.test_batch_size)
        valid_probe_batch, valid_probe_label = vgg.valid_batch_inputs(train_flags.dataset_valid_probe_csv_file_path,
...
def predict(gallery_flag):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            end_len = features_num % batch_len
            for batch_index in range(0, features_num - end_len, batch_len):
                batch_feature, batch_label = sess.run([vgg.fc7, predict_label])
                features[batch_index: batch_index + batch_len, :] = batch_feature
                labels[batch_index: batch_index + batch_len] = batch_label
