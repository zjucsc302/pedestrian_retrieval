...
class Vgg19:
    ...
    def train_batch_inputs(self, dataset_csv_file_path, batch_size):
        ...
        with tf.name_scope('batch_processing'):
            ...
            ref = tf.image.decode_jpg(ref_image, channels=3)
            ref = tf.cast(ref, tf.float32)

            ref_pos_image = tf.read_file(ref_pos_image_path)
            pos = tf.image.decode_jpg(ref_pos_image, dtype=tf.uint16, channels=1)
            pos = tf.cast(pos, dtype=tf.int16)

            ref_neg_image = tf.read_file(ref_neg_image_path)
            neg = tf.image.decode_jpg(ref_neg_image, dtype=tf.uint16, channels=1)
